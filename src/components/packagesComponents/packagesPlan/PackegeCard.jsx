...
const PackageCard = ({ subHeader, mainHeader, services, recommended }) => {
  return (
    <div className="border rounded-xl shadow-lg p-6 px-10 bg-white text-gray-800 flex flex-col justify-between">
      {/* Header Section */}
      <div>
        <h3 className="text-sm md:text-base font-semibold text-gray-800 text-center">
          {subHeader}
        </h3>
        <h2 className="text-2xl md:text-3xl font-bold text-[#363636] mb-4 text-center py-3">
          {mainHeader}
        </h2>
      </div>
      ...
      <div className="border-b border-gray-300 pt-4">
        <div className="w-full md:w-2/3 mx-auto mb-10">
          {/* <Button
            children={"Get Plan"}
          ...
          /> */}
          <button className="bg-yellow-400 hover:bg-yellow-500 text-black text-sm md:text-base font-semibold py-2 px-4 rounded-md w-full mb-3 transition-colors duration-300">
            Get Plan
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-grow justify-between">
        <ul className="mb-6 mt-6">
          {services?.map((service, index) => (
            <li
              key={index}
              className="mb-2 flex items-start gap-2 text-black font-semibold"
            >
              <FaCheck className="text-red-600 mt-1 flex-shrink-0" />
              <span className="text-sm md:text-base py-1">{service}</span>
            </li>
          ))}
        </ul>
